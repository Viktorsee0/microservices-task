...
> with the values of the passwords defined in the [`.env`](IdeaProjects/microservices-task/docker/.env) file (_"changeme"_ by default). The first one is the
...
To use a different version of the core Elastic components, simply change the version number inside the [`.env`](IdeaProjects/microservices-task/docker/.env)
...
A few extensions are available inside the [`extensions`](IdeaProjects/microservices-task/docker/extensions) directory. These extensions provide features which
...
[config-es]: IdeaProjects/microservices-task/docker/elasticsearch/config/elasticsearch.yml
[config-kbn]: IdeaProjects/microservices-task/docker/kibana/config/kibana.yml
[config-ls]: IdeaProjects/microservices-task/docker/logstash/config/logstash.yml
